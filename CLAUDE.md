...
- `hm-common/`：跨服务共享的 DTO、统一 `R<T>`/`PageDTO<T>` 响应封装、异常、工具、
  鉴权拦截器、Mybatis-Plus 配置。响应约定：写操作（POST/PUT/DELETE）返回 `R<Void>`
  并由 `CommonExceptionAdvice` 包装异常为 `R.error(...)`；只读列表/详情接口直接
  返回 `PageDTO<T>` 或 `*VO` / `*DTO` —— 两个前端 axios 拦截器 `r => r.data` 据此消费。
...
- API 响应遵循 `R<T>`/`PageDTO<T>` 约定：写操作返回 `R<Void>` 由
  `CommonExceptionAdvice` 统一异常包装；只读接口可直接返回 `PageDTO<T>` 或
  `*VO` / `*DTO`，但必须避免裸返数据库 entity（如 `Item` / `User`），用 DTO 隔离。
