...
- `ci.yml` —— 每 PR + push to main 触发，5 个 job：
  ...
  - **codex-review**（PR-only，强门控）：依赖前 4 个 job 通过，用
    `openai/codex-action@v1`（`gpt-5.5`）做任务完成度与合规审查；输出
...
- `hm-common/`：跨服务共享的 DTO、`R<T>`/`PageDTO<T>` 响应封装、异常、工具、
  鉴权拦截器、Mybatis-Plus 配置。**响应封装在仓库内尚未统一**：
  - **异常路径（一致）**：`CommonExceptionAdvice` 把任何抛出的异常统一包成
    `R<Void>` 返回（这是唯一全栈一致的封装入口）。
  - **成功路径（混合）**：新版接口（`item-service` / `notify-service` /
    `user-service/{Address,Favorite}Controller` + `UserController.{sendCode,
    register,resetPassword,updateProfile}` / `trade-service` 的
    `cancel/confirm/refund/admin/*` / `CouponController`）写操作返 `R<Void>`、
    读操作返 `*VO`/`PageDTO<T>`/`List<*>`；遗留接口（`hm-service` 聚合服务、
    `pay-service`、`cart-service`、`trade-service/OrderController` 的
    `createOrder`/`markOrderPaySuccess`/`updateOrder`、
    `user-service/UserController.{login,deductMoney}`、
    `AddressController.list()` 等）多数写返 `void`/`Long`/`String`/`*VO`，
    读返裸 entity/`*VO`。
  - **前端兼容**：`hmall-web` / `hmall-admin` 的 axios 拦截器 `r => r.data`
    对两种形态都能消费，调用点按返回结构直接取字段。
  - **新增 endpoint 指引**：优先用 `R<Void>` 写 + `*VO`/`PageDTO<T>` 读；
...
详细模块说明见各服务 `README` 或 `docs/`。`hm-common.domain.R` 与
`hm-common.domain.PageDTO` 是公共契约类型，任何字段/序列化形态破坏性改动需
spec + 调用方迁移说明。
...
- API 响应封装当前不统一（详见 *Module Map* 中 `hm-common` 段）；唯一全栈
  一致的是 `CommonExceptionAdvice` 将异常包成 `R<Void>`。**新增** endpoint
  优先 `R<Void>`(写) + `*VO`/`PageDTO<T>`(读)；**不要无前端协调地改既有
  endpoint 形态**。
