  ...
  <div>
    ...
    <el-table :data="feedbacks" style="margin-top: 16px"
      ...
      ><el-table-column label="操作" width="120"
        ><template #default="{ row }"
          ><el-button v-if="row.status === 0" size="small" type="primary" @click="showReply(row)"
            >回复</el-button
          ></template
        ></el-table-column
      ></el-table
    ...
    <el-dialog v-model="replyVisible" title="回复反馈"
      ><el-input v-model="replyContent" type="textarea" :rows="3" /><template #footer
        ><el-button type="primary" @click="doReply">发送回复</el-button></template
      ></el-dialog
    >
  </div>
</template>
...
async function doReply() {
  try {
    await replyFeedback(currentRow.value.id, { reply: replyContent.value });
