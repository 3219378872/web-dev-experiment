  ...
  <div>
    ...
    <el-table :data="orders" style="margin-top: 16px">
      ...
      <el-table-column label="金额" width="120"
        ><template #default="{ row }"
          >&yen;{{ (row.totalFee / 100).toFixed(2) }}</template
        ></el-table-column
      >
      <el-table-column label="状态" width="100"
        ><template #default="{ row }">{{
          ['', '待支付', '已支付', '已发货', '已完成', '已取消', '退款中'][row.status] || '-'
      ...
      <el-table-column label="操作">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 2" size="small" type="primary" @click="showShip(row)"
            >发货</el-button
          >
    ...
    <el-dialog v-model="shipVisible" title="填写物流单号"
      ...
      <template #footer
        ...
        ><el-button type="primary" @click="doShip">确认发货</el-button></template
      >
    </el-dialog>
  </div>
</template>
...
async function fetch() {
  try {
    const r = await getOrders({ page: 1, size: 50 });
    orders.value = r.list || [];
  } catch (err) {
    console.error(err);
  }
