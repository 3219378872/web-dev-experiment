...
sync_note: "fix(#155): dev/prod 前端入口转发根相对 /files 与 /upload 到 hm-gateway，上传图片 URL 无需前端改写即可显示。"
---
...
- 上传接口返回的图片 URL 是根相对 `/files/<key>`；Vite dev proxy 与生产 nginx
  必须把 `/files/**`、`/upload/**` 原样转发到 [hm-gateway](hm-gateway.md)，不要加 `/api`
  rewrite。
