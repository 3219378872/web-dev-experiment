...
sync_note: "fix(#140,#141): 管理端 /admin/orders 和 /admin/orders/export 支持 userId 过滤；订单详情继续复用 /orders/{id}/logistics 和 /admin/orders/{id}/refund-audit。"
---
...
- `TradeApplication.java`、`controller/OrderController.java`、
  `controller/CouponController.java`、`controller/DashboardController.java`。
- `service/IOrderService.java` / `ICouponService.java` / `ILogisticsTraceService.java` /
  `IDashboardService.java` 与 impl。
...
- `service/impl/OrderServiceImpl.java` —— 订单创建、MQ 发布、支付成功消费、延时关单、
  状态变更发布、退款审核。
- `service/impl/DashboardServiceImpl.java` —— 数据看板聚合（概览/趋势/品类/热销/待办）。
...
- 管理端订单列表与 CSV 导出共用过滤语义：`orderId` 精确匹配订单号、
  `userId` 精确匹配买家、`status` 匹配订单状态，导出不分页。
- 管理端订单详情的物流展示复用 C 端 `GET /orders/{id}/logistics`，返回
  `LogisticsTraceVO`；退款审核复用 `/admin/orders/{id}/refund-audit`。
