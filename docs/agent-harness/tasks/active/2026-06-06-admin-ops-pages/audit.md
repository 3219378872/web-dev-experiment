...
| #143 avatar upload is wired | done | `Profile.vue` uploads avatar with existing `POST /upload/image` helper, then persists avatar through `PUT /admin/profile`; helper tests cover payload shape. |
