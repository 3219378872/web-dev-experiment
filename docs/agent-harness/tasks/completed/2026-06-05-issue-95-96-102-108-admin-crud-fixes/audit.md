...
| #102 接入 Banner API | done | BannerList.vue 使用 getBanners/saveBanner/updateBanner/deleteBanner/updateBannerStatus；编辑离线 banner 时保留原 status |
...
| #108 发布设置持久化 | done | Item.java/ItemDTO.java 新增 isSeckill/isRecommend/isSevenDayReturn；前端 form reactive 字段绑定后端 DTO；新增迁移脚本 docs/sql/item-seckill-recommend-fields.sql |
| 无硬编码密钥 | done | 审查所有变更文件，无新增密钥 |
| API 响应形态兼容 | done | 新增 /admin/items/stats 返回 Map；现有接口形态未破坏 |
| Code review 通过 | done | codex-review 3 个 blocking findings 已全部修复 |
