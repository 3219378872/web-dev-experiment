      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane label="登录" name="login">
          <el-form :model="loginForm" label-width="80px">
            ...
            <el-form-item><el-button type="primary" @click="doLogin" :loading="loading">登录</el-button></el-form-item>
          </el-form>
        </el-tab-pane>
        <el-tab-pane label="注册" name="register">
          <el-form :model="registerForm" label-width="80px">
            ...
            <el-form-item label="验证码">
              ...
              <el-button :disabled="codeCountdown > 0" @click="sendRegisterCode" style="margin-left:8px">
                {{ codeCountdown > 0 ? codeCountdown + 's' : '发送验证码' }}
              </el-button>
            </el-form-item>
            <el-form-item><el-button type="primary" @click="doRegister">注册</el-button></el-form-item>
          </el-form>
        </el-tab-pane>
        <el-tab-pane label="找回密码" name="reset">
          <el-form :model="resetForm" label-width="80px">
            ...
            <el-form-item label="验证码">
              ...
              <el-button :disabled="codeCountdown > 0" @click="sendResetCode" style="margin-left:8px">
                {{ codeCountdown > 0 ? codeCountdown + 's' : '发送验证码' }}
              </el-button>
            </el-form-item>
            <el-form-item label="新密码"><el-input v-model="resetForm.newPassword" type="password" /></el-form-item>
            <el-form-item><el-button type="primary" @click="doReset">重置密码</el-button></el-form-item>
          </el-form>
        </el-tab-pane>
      </el-tabs>
    </el-card>
...
async function doLogin() {
  loading.value = true
  try { await userStore.login(loginForm); router.push('/'); ElMessage.success('登录成功') }
...
async function sendRegisterCode() {
  try { await sendCode(registerForm.email); startCountdown(); ElMessage.success('验证码已发送') } catch {}
}

async function sendResetCode() {
  try { await sendCode(resetForm.email); startCountdown(); ElMessage.success('验证码已发送') } catch {}
}

async function doRegister() {
  try { await register(registerForm); ElMessage.success('注册成功，请登录'); activeTab.value = 'login' } catch {}
}

async function doReset() {
  try { await resetPassword(resetForm); ElMessage.success('密码重置成功，请登录'); activeTab.value = 'login' } catch {}
}
</script>
