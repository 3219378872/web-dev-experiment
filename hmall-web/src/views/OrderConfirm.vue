<template>
  <div class="order-confirm">
    ...
    <el-card class="section-card"
      ><template #header>选择收货地址</template>
        ...
        <el-radio-group v-model="selectedAddress">
          <div v-for="addr in addresses" :key="addr.id" style="margin: 8px 0">
            <el-radio :label="addr.id">
              {{ addr.receiverName }} {{ addr.phone }} {{ addr.province }}{{ addr.city
    ...
    <el-card class="section-card"
      ...
      <el-table v-if="cartStore.selectedItems.length" :data="cartStore.selectedItems">
        ...
        <el-table-column label="数量"
          ><template #default="{ row }">x{{ row.num }}</template></el-table-column
        >
...
async function submitOrder() {
  ...
  try {
    await createOrder({ paymentType: 2, details });
