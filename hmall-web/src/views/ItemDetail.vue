<template>
  <div v-if="item" class="item-detail">
    <el-row :gutter="40">
      <el-col :md="10"
        ><img :src="item.image || '/placeholder.png'" class="detail-image" :alt="item.name"
      /></el-col>
      <el-col :md="14">
        <h1 class="item-name">{{ item.name }}</h1>
        <p class="item-price">&yen;{{ (item.price / 100).toFixed(2) }}</p>
        <p class="item-desc">
          库存: {{ item.stock }} | 已售: {{ item.sold }} | 评分: {{ item.commentCount }}条评价
        </p>
    ...
    <div class="review-section">
      ...
      <div v-if="reviews.length" class="review-list">
        <div v-for="r in reviews" :key="r.id" class="review-item">
          <p>
            <StarRating :model-value="r.rating" /><span class="review-time">{{
